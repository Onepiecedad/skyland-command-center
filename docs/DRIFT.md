...
- **Alex minne visar alltid tomt (9 sep).** `routes/alexMemory.ts` läser `MEMORY.md` och `memory/*.md` ur `OPENCLAW_WORKSPACE || HOME`. Två fel på en gång: backendn kör på Render där den katalogen inte finns, och på VPS:en där OpenClaw faktiskt kör innehåller `~/.openclaw/memory/` bara `*.sqlite`, ingen markdown, och `MEMORY.md` finns inte alls. Endpointen svarar alltså tomt oavsett maskin. Samma felfamilj som skills-katalogen nedan. Panelen säger nu varför i stället för "Inga minnen ännu", men den riktiga fixen är att läsa minnet ur sqlite-filerna eller över gateway-anslutningen. **Tomt i panelen är inget bevis på att Alex saknar minne.**
