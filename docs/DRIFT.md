...
> Senast verifierad: **2026-09-13** (Claude + Joakim: bumpen skrevs aldrig och motorn gick vidare till avslutet; OPENCLAW_HOME-krocken som fällde nattens research tre nätter).
...
| Röst i SCC:s flytande Alex-panel (håll-och-prata) | Samma två ändpunkter som WhatsApp-walkie-talkien, men från webbläsaren: mikrofonknapp i inmatningsraden (eller håll mellanslag) → `MediaRecorder` (webm/opus) → `/voice/stt` → transkriptet går in i **exakt samma** `/api/v1/chat/chat` och `conversation_id` som textfältet → svaret läses upp via `/voice/alex-tts` (markdown avskalat, klipps vid meningsslut före 1 200-teckenstaket). Alltså samma Alex, samma rättigheter, samma information som textchatten; ingen egen agent. Hook: `frontend/src/hooks/useWalkieTalkie.ts`. Den gamla ElevenLabs Conversational-vyn (`VoiceChat.tsx`, mäklaragenten) är urkopplad ur docken. | SCC (Render) | live 9 sep `b5a0dc4`, STT+TTS verifierade med riktigt ljud |
| Alex-panelens överlämning till VPS-Alex (`delegate_task`) | Panelens Alex äger CRM, skärm och saldo; research, prospektering, inkorg, annonser, filer och cron bor hos huvudagenten på VPS:en. Verktyget `delegate_task` köar uppdraget som en task med executor **`claw:main`**; `scc_poller` (var 15:e s) plockar den via `GET /claw/pending`, kör agenten `main` och rapporterar via `POST /claw/task-result`. Kom uppdraget från panelen (`input.source='panel'`) skickas svaret vidare som `ui_action { action:'note' }` på SSE — panelen lägger det i tråden och läser upp det. **Push är omöjlig**: gatewayen är loopback-bunden och nås bara över tailnetet, Render ligger utanför (`/api/v1/gateway/status` svarar `Gateway unreachable` från servern — det är väntat, inte ett fel). Kräver `OPENCLAW_DISPATCH_MODE=pull` och raden `"main": "main"` i pollerns `AGENT_MAPPING`. | SCC + VPS | live 9 sep, hela kedjan verifierad (23 s från fråga till klar körning) |
| Strömmande svar i Alex-panelen | `POST /api/v1/chat/stream` kör samma pipeline som `/chat/chat` men skickar texten bit för bit (SSE över POST: `delta`, `round`, `final`, `error`). OpenRouter-adaptern kör `stream: true` när `onDelta` finns och sätter ihop verktygsanropen per index; `round` betyder ny LLM-runda, dvs. det strömmade före var förspel till ett verktygsanrop och bufferten ska nollställas. Panelen skriver ut texten medan den kommer och `voice/speechQueue.ts` läser upp färdiga meningar i ordning medan nästa hämtas. Mätt live: första tecknet 2,6 s, första hela meningen 5,0 s, hela svaret 12,4 s — uppläsningen startar alltså vid 5 s i stället för 13,5 s. `/chat/chat` är oförändrad och används fortfarande av röstwebhooken. | SCC (Render) | live 9 sep, verifierad med och utan verktygsanrop |
| Alex säger till av sig själv (`pulse`) | `services/pulse.ts`, var 60:e s (`PULSE_ENABLED`, `PULSE_INTERVAL_MS`). Tre källor: nytt `ce_leads`-lead, OpenRouter-saldo under 5 USD, och något nere (integration `down`/`auth_failed` eller stillastående poller). Skickar `ui_action { action:'note', source:'pulse', kind, speak }` — samma rör som delegeringens svar. **Karens:** första ronden efter omstart lär sig läget utan att larma (annars skriks historiken ut vid varje deploy), ett lead sägs en gång, saldo max var 6:e timme, nere-läget bara vid FÖRÄNDRING (och tystnad när det återställs). Panelen läser upp självmant tal bara om operatören varit aktiv senaste 15 min, panelen är öppen och inget annat redan låter; annars hamnar raden bara i tråden. | SCC (Render) | live 9 sep, verifierad skarpt: lead in 18:58:44 → uppläst rad 18:59:28 |
...
- **Tester som beror på vem som kör dem (13 sep).** `routes/automations.test.ts` hade sex röda tester på Joakims Mac och gröna i CI. Rutten väljer väg på `fs.existsSync(DB)`, alltså om OpenClaws sqlite finns: på Render gör den inte det och knappen köas (202/404/500, det testerna hävdar), på Macen finns filen och samma rutt pratar med den lokala gatewayn och svarar 200. Testerna öppnade dessutom hans riktiga sqlite och ringde hans riktiga gateway. Preconditionen mockas nu i testfilen, så filen testar samma sak överallt (11/11, 59 ms i stället för 5 s). `routes/skills.test.ts` var röd av samma familj — den slår mot riktig DB — och är grön när databasen är nåbar. **Mönstret att leta efter: ett test vars utfall beror på vad som råkar ligga i hemkatalogen är inte ett test.** Hela sviten: 498 gröna.
...
- ~~`GET /api/v1/website/workflows` pekar på n8n:s API.~~ **Riven 13 sep.** Den svarade 502, inte 503 som saknad config skulle gett — alltså låg `N8N_API_URL`/`N8N_API_KEY` kvar i Render och pekade på den uppsagda tjänsten, vilket fick arvet att se ut som ett driftfel. Endpoint, api-klient, typ, panelen i Sajt-vyn och propen `visaWorkflows` är borta. Cron-jobbens hälsa i Systemvyn är orörd (den läser `/api/v1/automations`). **Kvar för Joakim: ta bort de två N8N-variablerna ur Render.**
...
- **Agenter och MCP i Capabilities (9 sep).** Filtren letade `source === 'subagent'` respektive `'mcp'`, men gatewayns `skills.status` sätter aldrig fältet. `/api/v1/skills-aggregator` gör det rätt — den läser subagentkatalogerna och `mcp_config.json`. **Den svarade 404 fram till 13 sep:** routern flyttades till sin egen prefix i `server.ts` men behöll vägen `/skills` från legacy-`index.ts`, där den satt på `/api/v1`. Riktig adress blev `/skills-aggregator/skills` medan frontenden ropade på `/skills-aggregator`. Rutten ligger nu i rotens plats och svarar 200; träffarna slås ihop med gatewayns skills. Den läser filer på maskinen där OpenClaw kör, så på Render svarar den tomt, och då göms flikarna i stället för att visa nollor i evighet. Samma familj som skills-katalogen: filer på operatörens dator finns inte i molnet.
...
- **Annonsmotorn: egna siffror möter konkurrentdatat (9 sep).** `ad_library` hade 4 161 konkurrentannonser men det fanns ingen tabell alls för VÅRA egna annonser. Ingen spend, ingen CTR, inget resultat, alltså inget sätt att kvalitetssäkra en annons efter publicering. Nu finns `ad_accounts`, `ad_performance`, `ad_targets` samt vyerna `ad_health` (grönt/gult/rött) och `ad_funnel` (annonskostnad mot CRM-utfall via `ad_id`). Edge-funktionen `ads-sync` hämtar Metas insights på annonsnivå, cron-jobbet `ads_sync_daglig` kör 06:15 UTC. Full uppsättning i `docs/ANNONSMOTOR.md`.
  **Två återvändsgränder, dokumenterade så de inte testas om.** *Windsor.ai* har noll kopplade konton och ligger på gratisplanen; `get_connectors` returnerar tom lista. Jag påstod i chatten att Windsor "redan är kopplat och läser Meta Ads" — det var fel. *`META_PAGE_TOKEN`* är en **sid**token: `/me` löser till sidan Cold Experience, så `/me/adaccounts` och `/me/businesses` svarar `(#100) Tried accessing nonexisting field`. En sidtoken kan aldrig läsa annonsstatistik. Rätt väg är en systemanvändartoken med `ads_read`.
  **`ads-sync` felar stängt.** Utan både `META_ADS_TOKEN` och `ADS_SYNC_KEY` svarar den 503 och gör ingenting. Nyckeln skickas som `x-ads-key`; cron läser sin kopia ur vault och hoppar tyst över om den saknas. Verifierat: anrop utan nycklar ger 503, inte en öppen endpoint.
  **Hämtningsfönstret är tre dagar, inte en.** Meta räknar om resultat i efterhand när attributionsfönstret stänger, så gårdagens siffra är inte slutgiltig. Upsert på `(platform, ad_id, date)` gör omhämtningen gratis.
  **Grått är inte rött i `ad_health`.** Fyra färger, inte tre: grått betyder att annonsen inte spenderat upp till sitt `spend_floor` och alltså inte kan dömas än. Att pausa en annons som knappt fått pengar är det vanligaste sättet att döda något som hade fungerat.
  **`ad_funnel` visar redan en rad utan att en siffra hämtats:** 77 av 79 Cold Experience-leads kommer från EN annons (`120250607553950299`), 30 heta, noll överlämnade, noll bokade. Sista två är en signal om uppföljningen, inte ett datafel.
  **Ingen auto-paus, med flit.** Med tre kunder och en handfull aktiva annonser är en flagga och ett mänskligt beslut snabbare och säkrare än en regel som pausar på brus. Bygg det först när volymen motiverar det, och då med spendgolv, loggat skäl i klartext och en ångraknapp.
- **`customers` och `tenants` är två parallella register (noterat 9 sep).** `tenants` har 7 rader (Cold Experience, MarinMekaniker, tre mäklare, Skyland internt), `customers` har 5 (Hasselblads, MarinMekaniker, Cold Experience, All Gold Tattoo, GKMK). De överlappar delvis: MarinMekaniker finns i båda, Cold Experience bara i tenants, GKMK och All Gold bara i customers. `ce_leads.tenant_id` pekar på tenants, `ad_accounts.customer_id` på customers. `ad_funnel` undviker problemet genom att joina enbart på `ad_id`, som är globalt unikt hos Meta. **Samma felfamilj som `ce_*`: två register för samma sak.** Rör inte förrän det finns ett beslut om vilket som är sanningen.
- **`ad-intel`-skillen dokumenterar en flagga som inte finns (9 sep).** SKILL.md beskriver `--patterns` utförligt, inklusive språkfilter och `--all-langs`. Scriptet `scripts/ad_intel.py` har ingen sådan flagga. Antingen togs den bort eller så skrevs dokumentationen före koden. `--report` fungerar. Vertikalerna `arctic` och `kampsport` lades till i `choices` 9 sep; `arctic` fanns redan i tabellen men gick bara att fylla via andra script.
  **Kampsport-sweepen kördes 9 sep:** 487 annonser, 211 annonsörer, 20 aktiva, 50 långkörare. **Göteborgs Krav Maga Klubb är femte största annonsör med 10 annonser men noll aktiva just nu.** Andra Göteborgsklubbar som annonserar: Kali Sikaran Göteborg (73 dagar), Hisingens Kampsportscenter (20 dagar), Masthuggets Kampsportsklubb. Längsta långköraren i landet är Jabb.se på 442 dagar.
- **Alla Messenger-leads saknade namn, och Meta har namnen (9 sep).** Varje messenger-lead sedan starten låg med `name = null` och stod som "Okänt namn" i CRM:et. Orsaken: `getMessengerProfile` gick mot User Profile API:t, `GET /{PSID}?fields=first_name,last_name`, som svarar 400 med kod 100 subkod 33, "does not exist, cannot be loaded due to missing permissions". Det kräver App Review-funktionen **Business Asset User Profile Access**, som vi inte har och inte behöver skaffa. Webhook-payloaden bär bara `sender.id`, inget namn, så koden hade ingen annan källa.
  **Conversations-API:t svarar 200 med exakt samma sidtoken.** `GET /{page-id}/conversations?user_id={PSID}&fields=participants` returnerar deltagarna i tråden, och gästen är den vars id inte är sidans. Ingen App Review, ingen ny token, ingen ny behörighet. Verifierat mot alla åtta befintliga trådar: sju svarade, en var arkiverad av avsändaren ("The thread owner has archived or deleted this conversation"). Sex kort backfylldes, och speglingen till `contacts` följde med av sig själv. **`Facebook user` är Metas platshållare för ett borttaget eller spärrat konto och skrivs aldrig in som namn.** Namnet fylls numera också på i efterhand vid nästa inkommande meddelande om det saknas.
  **Diagnostiken kördes som en tillfällig edge-funktion, `mp-probe`, som läste `META_PAGE_TOKEN` serverside och bara returnerade statuskoder och namn. Den är raderad.** Metoden är värd att komma ihåg: när frågan är "vad svarar Meta med den token vi faktiskt har", testa serverside i stället för att gissa utifrån dokumentationen.
- **Samma gäst låg som två leads: formulär plus Messenger (9 sep).** När någon fyller i annonsformuläret skickar Meta automatiskt ett meddelande i gästens namn, `Hello! I filled out your form and would like to know more about your business`, typiskt 8 till 14 sekunder senare. Det landade som en separat messenger-tråd utan namn, mejl eller telefon, medan formulärskortet hade allt utom samtalet. Gustav såg alltså ett kort han kunde ringa och ett kort med konversationen, utan koppling. Fem par i basen: Katelijn Herrebosch, FC Van Der Merwe, Lerato Moletsane, Sandra De Vos och Nicky Theunissen. Sammanslagna för hand i en transaktion: konversationer, meddelanden och händelser flyttade till formulärsleadet, psid:et satt där, `leads_merged` loggad, speglade `contacts` och `opportunities` för stubben raderade, stubben raderad. 84 leads → 79, noll föräldralösa rader efteråt.
  **`ce_lead_events` går inte att flytta med `app.allow_event_purge`** — den escape-hatchen täcker bara DELETE, vakten stoppar UPDATE villkorslöst. Lösningen var `ALTER TABLE ce_lead_events DISABLE TRIGGER trg_ce_events_no_update` inuti samma DO-block, med ENABLE sist. DDL är transaktionell i Postgres, så en avbruten körning lämnar aldrig vakten avstängd. Gör aldrig det här i två separata anrop.
  **`ce_mirror_message` uppdaterar inte `customer_id`** när ett meddelande flyttas, bara `content`, `status` och `metadata`. Speglade rader måste rättas separat, annars pekar de på fel kund.
  **Koden gör hopslagningen själv nu.** Före ett nytt messenger-kort skapas letas formulärsleadet upp på namn, eller på att tråden öppnas med Metas formulärsmening inom två minuter. Båda kräver exakt en träff och att kandidaten saknar psid. Tidsmatchningen behövs eftersom "FC Van Der Merwe" på Facebook skrev "FC vd Merwe" i formuläret.
  **Kvar som rena messenger-leads:** tre. En är ett bedrägeriförsök, "⚠️ Important Notice from Meta Policy Support Team", avsändare Deon Sungwa. Två saknar fortfarande namn: en arkiverad tråd och ett konto Meta bara kallar "Facebook user".
  **Det överlevande kortet behåller `channel` från formuläret**, alltså `email` eller `whatsapp`, trots att samtalet nu är i Messenger. Agenten svarar på den kanal meddelandet kom in på, inte på `channel`, så det fungerar, men kortet kan visa fel kanaletikett. Rör det inte utan att först kolla att mejlsekvensen inte hänger på fältet.
- **Uppgifter-fliken i Alex var hårdkodad tom (9 sep).** Panelen renderade "Inga aktiva uppgifter just nu" som statisk text: ingen fetch, inget state, ingen datakälla. Statusraden längst ned hämtade samtidigt riktiga uppgifter och skrev "1 aktiva uppgifter", vilket är motsägelsen som ledde hit. Panelen läser nu `fetchTasks` för `in_progress`, `assigned` och `review` var 30:e sekund, raderna öppnar `TaskDetail` och en uppgift som stått stilla i över ett dygn märks med hur länge och "fastnat?". Samma familj som kostnadsöversikten: en platshållare som stod framför ett fungerande API.
  **Statusraden anropade fel adress.** `StatusBar.tsx` byggde `${API_URL}/api/v1/tasks` med rå `fetch`, alltså dubbelt `/api/v1` i varje läge där `API_URL` redan pekar på basen, och utan auth-huvud. Den går nu genom `fetchTasks` som resten av frontenden.
  **Den enda "aktiva" uppgiften är en zombie.** "Producera Paket 1 för Skinspiration Tattoo", executor `claw:produce-package`, `in_progress` sedan 24 juli, alltså 47 dagar. En syskonrad med identisk titel är `failed` en kvart tidigare samma dag. Ingen kör den. Databasen har totalt 6 uppgifter: 3 misslyckade, 2 klara, den här. Statusen är inte ändrad — det är ett databeslut, inte ett buggfix — men så länge den står kvar räknar statusraden en fastnad rad som pågående arbete.
